...
checkpoint = f"{checkpoint_folder}/esm_if1_gvp4_t16_142M_UR50.pt"
...
if '.csv' not in dms_input:
    ...
    for idx in df.index:
        ...
        #os.path.exists(f'./output/{DMS_id}.csv') or 
        if os.path.exists(f'./output/{DMS_id}.csv'):
            print(DMS_id)
            continue
        i = idx % gpu_count
        gpu_id = gpus[i]
        cmd = f'CUDA_VISIBLE_DEVICES={gpu_id} {python} {dir_path}/../../baselines/esm/compute_fitness_esm_if1_multi_pdb.py' \
