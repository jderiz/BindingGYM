...
if '.csv' not in dms_input:
    ...
    for idx in df.index:
        ...
        i = idx % gpu_count
        gpu_id = gpus[i]
        cmd = f'CUDA_VISIBLE_DEVICES={gpu_id} {python} {dir_path}/../../baselines/trancepteve/score_trancepteve_multi_pdb.py' \
            + f' --dms_index {idx}' \
            + f' --dms_mapping {dms_mapping}' \
            + f' --dms_input {dms_input}' \
            + f' --dms_output {dms_output}' \
            + f' --inference_time_retrieval_type TranceptEVE' \
            + f' --EVE_num_samples_log_proba 2000' \
            + f' --EVE_model_folder {eve_model_path}' \
            + f' --EVE_model_parameters_location {model_parameters_location}' \
            + f' --checkpoint {checkpoint}' \
            + f' --scoring_window optimal' \
            + f' --msa_path {msa_path}' \
            + f' --msa_db_path {msa_db_path}' \
            + f' --a2m_root {a2m_root}' \
            + f' --batch_size 8' \
            + f' --EVE_seeds 42' \
            + f' --EVE_recalibrate_probas'  
    ...
    cmd = f'CUDA_VISIBLE_DEVICES={gpu_id} {python} {dir_path}/../../baselines/trancepteve/score_trancepteve_multi_pdb.py' \
            ...
            + f' --dms_output {dms_output}' \
            + f' --inference_time_retrieval_type TranceptEVE' \
            + f' --EVE_num_samples_log_proba 200000' \
            + f' --EVE_model_folder {eve_model_path}' \
            + f' --EVE_model_parameters_location {model_parameters_location}' \
            + f' --checkpoint {checkpoint}' \
            + f' --scoring_window optimal' \
