...
class Scorer:
    ...
    def _featurize(self,raw_batch):
        batch = self.alphabet.featurize(raw_batch=raw_batch)
        ...
        batch['prev_token_mask'] = batch['tokens'].eq(self.alphabet.mask_idx)
        # batch['prev_tokens'] = prev_tokens
        # batch['prev_token_mask'] = prev_tokens.eq(self.alphabet.mask_idx)
        self.batch = batch
        return batch

    def score(self,batch,decoding_order=None):
        # 0) encoding
        encoder_logits, encoder_out = self.model.encoder(batch, decoding_order=decoding_order, return_feats=True)
        ...
        esm_logits = decoder_out['logits']


        
        return esm_logits.log_softmax(-1)
