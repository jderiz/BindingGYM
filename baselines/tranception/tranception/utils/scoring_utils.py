...
def get_sequence_slices(df, target_seq, model_context_len, start_idx=1, scoring_window="optimal", indel_mode=False):
    ...
    def get_mutant_pos(x):
        if x == '':
            return 0
        else:
            return int(np.array([int(mutation[1:-1]) - start_idx for mutation in x.split(':')]).mean())
    if scoring_window=="optimal":
        df['mutation_barycenter'] = df['mutant'].apply(get_mutant_pos) if not indel_mode else df['mutated_sequence'].apply(lambda x: len(x)//2)
